...
module.exports = class FunctionsAuthentication {
  ...
  static async authenticationVerification(strData, verificationType, _returnURL = '', specialParameters = {}) {
    ...
    // Variables.
    // ----------------------
    const objReturn = { statusReturn: false };
    let strDataDecrypt;
    let returnURL;

    const arrSearchParametersUsersDetailsAuthentication = [];
    let objUsersDetailsAuthentication;
    let objUsersDetailsAuthenticationParameters;

    let resultUsersDetailsAuthentication;
    // ----------------------
    ...
    if (strData) {
      ...
      if (verificationType === 'user_root') {
        ...
        */
        ...
        resultUsersDetailsAuthentication = await FunctionsDB.genericTableGet02(
          gSystemConfig.configSystemDBTableUsers,
