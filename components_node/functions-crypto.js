  ...
  static encryptValue(strValue, encryptMethod = 2) {
    ...
    if (encryptMethod == 2) {
      ...
      if (gSystemConfig.configCryptData == 23) {
        const cryptoKey = crypto.createCipher('aes-128-cbc', gSystemConfig.configCryptKey);
        const cryptoString = cryptoKey.update(strValue, 'utf8', 'hex');
      ...
      if (gSystemConfig.configCryptData == 26) {
        ...
        */
        ...
        const cryptoCipher = crypto.createCipheriv('aes-256-cbc', Buffer.from(this.bufCryptKey32ByteFormated), this.bufCryptiv16ByteFormated);
  ...
  static decryptValue(strValue, encryptMethod = 2) {
    ...
    try {
      ...
      if (encryptMethod == 2) {
        // Crypto Module algorithm: aes-256-cbc - 32 byte key and 16 byte iv
        if (gSystemConfig.configCryptData == 23) {
          const cryptoKey = crypto.createDecipher('aes-128-cbc', gSystemConfig.configCryptKey);
          const cryptoString = cryptoKey.update(strValue, 'hex', 'utf8');
        ...
        if (gSystemConfig.configCryptData == 26) {
          ...
          */
          ...
          // Logic.
          const cryptoiv = Buffer.from(this.bufCryptiv16ByteFormated.toString('hex'), 'hex');
          const encryptedString = Buffer.from(strValue, 'hex');
          const cryptDecipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(this.bufCryptKey32ByteFormated), cryptoiv);
      ...
      // TODO: maybe, move return to this part.
    }
