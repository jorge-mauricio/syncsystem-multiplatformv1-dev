...
class FrontendQuizzesListingRecord extends Component {
  ...
  handleQuizResultLog = (eventData) => {
    ...
    if (divQuizID == this.props.arrQuizzesListing.length - 1) {
      ...
      // (async function (quizResultsLog, idTbRegistersLogged) {
      (async (quizResultsLog, idTbRegistersLogged) => {
  ...
  render() {
    ...
    if (configLayoutType === 2) {
      if (arrQuizzesListing.length > 0) {
          ...
          <React.Fragment>
            ...
            <div className="ss-frontend-quizzes-listing-container">
              {arrQuizzesListing.map((quizzesRow, quizzesRowKey) => {
                return (
                  <article id={'divQuiz' + quizzesRowKey} key={quizzesRowKey} className="ss-frontend-quizzes-container ss-frontend-quizzes-listing-text01" style={{ display: 'none' }}>
                    <h2 className="ss-frontend-quizzes-listing-title01">
                      <a href={'/' + gSystemConfig.configRouteFrontendQuizzes + '/' + gSystemConfig.configRouteFrontendDetails + '/' + quizzesRow.id} className="ss-frontend-quizzes-listing-title-link01" title={SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageFrontend.appLabels, 'backendItemDetails')}>
                        {/* quizzesRow.title */}
            ...
            <div id={'divQuizResultsLog'} style={{ display: 'none' }}>
              <div>
                {SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageFrontend.appLabels, 'frontendQuizzesAnswersRight')}:{this.state.quizAnswersRight} / {arrQuizzesListing.length}
              </div>
              <div>
                ...
                {this.state.quizResultsLog.map((quizResultsLogRow, quizResultsLogKey) => {
                  return (
                    <div
                      key={quizResultsLogKey}
                      className={
                        // quizResultsLogRow.quizAnswerStatus.toString() == quizResultsLogRow.objQuizDetails.id ?
                        quizResultsLogRow.quizAnswerStatus === true ? `ss-frontend-correct` : `ss-frontend-incorrect`
                      }
                    >
                  ...
                  this.state.quizResultsLog.map((quizResultsLogRow, quizResultsLogKey) => {
                    return (
                      <div key={quizResultsLogKey}>
                        ...
                        <br />
                        Question:
                        <div className="ss-frontend-quizzes-listing-subheading01">{HTMLReactParser(quizResultsLogRow.objQuizDetails.title)}</div>
                        <br />
                        Status:
                        {/* Debug. */}
        ...
        // Output - empty.
        return <div className="ss-frontend-alert">{SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageFrontend.appLabels, 'frontendMessageQuizzesEmpty')}</div>;
      }
    }
    // ----------------------
  }
