...
module.exports = class QuizzesEdit {
  ...
  async build() {
    ...
    try {
      ...
      if (gSystemConfig.enableQuizzesStatus !== 0) {
        this.resultsQuizzesStatusListing = this.ofglRecords.resultsFiltersGenericListing.filter((obj) => {
      ...
      // Meta title.
      this.metaTitle += SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendQuizzesTitleEdit');
  ...
  async cphTitleBuild() {
    ...
    // ----------------------
    try {
      this.cphTitle = SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendQuizzesTitleEdit');
  ...
  async cphBodyBuild() {
    ...
    // Redefine values.
    let oqdRecord = this.oqdRecord; // eslint-disable-line
