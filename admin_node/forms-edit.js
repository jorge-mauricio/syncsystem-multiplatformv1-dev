...
module.exports = class FormsEdit {
    ...
    try {
      ...
      if (gSystemConfig.enableFormsIdParentEdit == 1) {
        ...
        if (this.objParentTableLevel1.tableName == gSystemConfig.configSystemDBTableCategories) {
          this.objParentTable = await SyncSystemNS.FunctionsDB.genericTableGet02(gSystemConfig.configSystemDBTableCategories, ['category_type;12;i'], gSystemConfig.configCategoriesSort, '', 'id, title', 1);
        }
      ...
      // Meta title.
      this.metaTitle += SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendFormsTitleEdit');
  ...
  async cphTitleBuild() {
    ...
    // ----------------------
    try {
      this.cphTitle = SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendFormsTitleEdit');
