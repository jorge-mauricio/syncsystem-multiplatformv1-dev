...
module.exports = class UsersEdit {
  ...
  async build() {
    ...
    try {
      ...
      if (gSystemConfig.enableUsersType !== 0) {
        this.resultsUsersTypeListing = this.ofglRecords.resultsFiltersGenericListing.filter((obj) => {
      ...
      if (gSystemConfig.enableUsersStatus !== 0) {
        this.resultsUsersStatusListing = this.ofglRecords.resultsFiltersGenericListing.filter((obj) => {
      ...
      // Meta title.
      this.metaTitle += SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendUsersTitleEdit');
  ...
  async cphTitleBuild() {
    ...
    // ----------------------
    try {
      this.cphTitle = SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendUsersTitleEdit');
  ...
  async cphBodyBuild() {
    ...
    // Redefine values.
    let oudRecord = this.oudRecord; // eslint-disable-line
    ...
    let resultsUsersTypeListing = this.resultsUsersTypeListing; // eslint-disable-line
    let resultsUsersStatusListing = this.resultsUsersStatusListing; // eslint-disable-line
