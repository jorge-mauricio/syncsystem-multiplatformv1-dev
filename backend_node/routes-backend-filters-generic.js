...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendFiltersGeneric + '/', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
...
router.post('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendFiltersGeneric, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  ...
  let tblFiltersGenericConfigSelection = 0; // eslint-disable-line
          ...
          form.on('end', async function (fields, files) {
            ...
            resultsFunctionsFiles = await new Promise((resolve, reject) => {
              /*SyncSystemNS.FunctionsFiles.filesUpload(tblCategoriesID, 
                                                                    ...
                                                                    gSystemConfig.configDirectoryFilesUpload, 
                                                                    "")*/
              SyncSystemNS.FunctionsFiles.filesUploadMultiple(tblFiltersGenericID, this.openedFiles, gSystemConfig.configDirectoryFilesUpload, '', formfileFieldsReference).then(function (results) {
                if (results === undefined) {
                  // Error.
                  if (gSystemConfig.configDebug === true) {
...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendFiltersGeneric + '/' + gSystemConfig.configRouteBackendActionEdit + '/:idTbFiltersGeneric?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Import objects.
  // ----------------------
...
router.put('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendFiltersGeneric + '/' + gSystemConfig.configRouteBackendActionEdit, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Variables
  // ----------------------
  let tblFiltersGenericID = '';
