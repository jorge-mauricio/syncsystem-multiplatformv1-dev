...
// Backend - Home - Login.
// **************************************************************************************
router.get('/' + gSystemConfig.configRouteBackend, (req, res, next) => {
...
router.post('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendLogin, (req, res, next) => {
  ...
  let pageNumber = ''; // eslint-disable-line
    ...
    try {
      ...
      if (loginVerification === true) {
        ...
        /*
                ...
                */
        let cookieOptions = gSystemConfig.configCookieDefaultOptions; // eslint-disable-line

        // Set cookie.
        res.cookie(gSystemConfig.configCookiePrefix + '_' + gSystemConfig.configCookiePrefixUser, tblUsersIDCrypt, cookieOptions);
...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendDashboard + '/', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Import objects.
  // ----------------------
...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendLogOff, [SyncSystemNS.FunctionsCookies.cookieDelete_middleware(gSystemConfig.configCookiePrefix + '_' + gSystemConfig.configCookiePrefixUser)], (req, res, next) => {
  ...
  // Define values.
  returnURL = '/' + gSystemConfig.configRouteBackend + '/?messageSuccess=statusMessageLogin2'; // eslint-disable-line
