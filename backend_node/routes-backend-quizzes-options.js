...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzesOptions + '/:idQuizzes?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
          ...
          form.on('end', async function (fields, files) {
            ...
            resultsFunctionsFiles = await new Promise((resolve, reject) => {
              ...
              SyncSystemNS.FunctionsFiles.filesUpload(tblCategoriesID, 
                                                                    ...
                                                                    gSystemConfig.configDirectoryFilesUpload, 
                                                                    "")*/
              // SyncSystemNS.FunctionsFiles.filesUploadMultiple(tblQuizzesOptionsID, this.openedFiles, gSystemConfig.configDirectoryFilesUpload, '', formfileFieldsReference).then(function (results) {
              // eslint-disable-next-line
...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzesOptions + '/' + gSystemConfig.configRouteBackendActionEdit + '/:idTbQuizzesOptions?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Import objects.
  // ----------------------
...
router.put('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzesOptions + '/' + gSystemConfig.configRouteBackendActionEdit, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Variables
  // ----------------------
  let tblQuizzesOptionsID = '';
