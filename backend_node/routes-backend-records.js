...
router.delete('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendRecords, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_root_or_user_backend')], (req, res, next) => {
  ...
  let messageSuccess = ''; // eslint-disable-line
  let messageError = ''; // eslint-disable-line
  let messageAlert = ''; // eslint-disable-line
    ...
    try {
      if (idsRecordsDelete) {
        ...
        if (Array.isArray(deleteRecordsFilesFields) && deleteRecordsFilesFields.length) {
          deleteRecordsFilesResult = await SyncSystemNS.FunctionsDB.genericTableGet02(
            strTable,
...
router.patch('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendRecords, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_root_or_user_backend')], (req, res, next) => {
  // working, with the async block
  // Variables.
  // ----------------------
  let objReturn = { returnStatus: false, nRecords: 0, recordUpdatedValue: null }; // eslint-disable-line
    ...
    try {
      ...
      if (configAPIKey === SyncSystemNS.FunctionsCrypto.decryptValue(SyncSystemNS.FunctionsGeneric.contentMaskRead(apiKey, 'env'), 2)) {
        if (patchType === 'setValue') {
          ...
          updateRecordsGeneric10Result = await SyncSystemNS.FunctionsDBUpdate.updateRecordGeneric10(strTable, strField, recordValue, ['id;' + idRecord + ';i']);
        ...
        if (patchType === 'toggleValue') {
          ...
          // Get current value.
          recordCurrentResult = await SyncSystemNS.FunctionsDB.genericTableGet02(strTable, ['id;' + idRecord + ';i'], '', '', 'id, ' + strField, 1, { returnType: 3 });
          ...
          // Udate record.
          updateRecordsGeneric10Result = await SyncSystemNS.FunctionsDBUpdate.updateRecordGeneric10(strTable, strField, valueUpdate, ['id;' + idRecord + ';i']);
        ...
        if (patchType == 'fileDelete') {
          ...
          // eslint-disable-next-line
          let resultsRecordDetails = await SyncSystemNS.FunctionsDB.genericTableGet02(strTable, ['id;' + idRecord + ';i'], '', '', strField, 1);
