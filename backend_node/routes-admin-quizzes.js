...
// **************************************************************************************
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzes + '/:idParent?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
...
router.post('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzes, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  ...
  let tblQuizzesIdRegisterUser = 0; // eslint-disable-line
      ...
      let formParseResults = await new Promise((resolve, reject) => {
        ...
        if (gSystemConfig.configUploadComponent === 1) {
          ...
          form.on('end', async function (fields, files) {
            ...
            /*
                        // let resultsFunctionsFiles = await SyncSystemNS.FunctionsFiles.filesUpload(tblQuizzesID, 
...
// **************************************************************************************
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzes + '/' + gSystemConfig.configRouteBackendActionEdit + '/:idTbQuizzes?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
...
// **************************************************************************************
router.put('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendQuizzes + '/' + gSystemConfig.configRouteBackendActionEdit, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Variables
  // ----------------------
  let tblQuizzesID = '';
