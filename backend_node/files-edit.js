...
module.exports = class FilesEdit {
    ...
    try {
      ...
      if (gSystemConfig.enableFilesIdParentEdit === 1) {
        ...
        if (this.objParentTableLevel1.tableName == gSystemConfig.configSystemDBTableCategories) {
          this.objParentTable = await SyncSystemNS.FunctionsDB.genericTableGet02(gSystemConfig.configSystemDBTableCategories, [], gSystemConfig.configCategoriesSort, '', 'id, title', 1);
        }
  ...
  async cphTitleBuild() {
    ...
    // ----------------------
    try {
      this.cphTitle = SyncSystemNS.FunctionsGeneric.contentMaskRead(gSystemConfig.configSystemClientName, 'config-application') + ' - ' + SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageBackend.appLabels, 'backendFilesTitleEdit');
  ...
  async cphBodyBuild() {
    ...
    // Redefine values.
    let ofdRecord = this.ofdRecord; // eslint-disable-line
