...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendUsers + '/:idParent', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_root')], (req, res, next) => {
          ...
          form.on('end', async function (fields, files) {
            ...
            resultsFunctionsFiles = await new Promise((resolve, reject) => {
              /*SyncSystemNS.FunctionsFiles.filesUpload(tblCategoriesID, 
                                                                    ...
                                                                    gSystemConfig.configDirectoryFilesUpload, 
                                                                    "")*/
              // SyncSystemNS.FunctionsFiles.filesUploadMultiple(tblUsersID, this.openedFiles, gSystemConfig.configDirectoryFilesUpload, '', formfileFieldsReference).then(function (results) {
              // eslint-disable-next-line
...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendUsers + '/' + gSystemConfig.configRouteBackendActionEdit + '/:idTbUsers?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_root')], (req, res, next) => {
  // working, with the async block
  // Import objects.
  // ----------------------
...
router.put('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendUsers + '/' + gSystemConfig.configRouteBackendActionEdit, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_root')], (req, res, next) => {
  // working, with the async block
  // Variables
  // ----------------------
  let tblUsersID = '';
