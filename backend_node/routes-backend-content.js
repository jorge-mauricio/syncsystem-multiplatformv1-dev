...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendContent + '/:idParent?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
...
router.post('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendContent, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  ...
  let tblContentFileDuration = ''; // eslint-disable-line
  let tblContentFileDimensions = ''; // eslint-disable-line
      ...
      let formParseResults = await new Promise((resolve, reject) => {
        ...
        if (gSystemConfig.configUploadComponent === 1) {
          ...
          // form.on('end', async (fields, files) => {
            ...
            // if(fieldsPost.file_config != 0) // Check if is content.
            if (fieldsPost.content_type == '5' || fieldsPost.content_type == '7' || fieldsPost.content_type == '8' || fieldsPost.content_type == '9' || fieldsPost.content_type == '11') {
              // Check if is content.
            ...
            if (_.isEmpty(formfileFieldsReference) !== true) {
              ...
              resultsFunctionsFiles = await new Promise((resolve, reject) => {
                ...
                SyncSystemNS.FunctionsFiles.filesUpload(tblCategoriesID, 
                                                                        ...
                                                                        "")*/
                // SyncSystemNS.FunctionsFiles.filesUploadMultiple(tblContentID, this.openedFiles, gSystemConfig.configDirectoryFilesUpload, '', formfileFieldsReference).then(function (results) {
                // eslint-disable-next-line
...
router.get('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendContent + '/' + gSystemConfig.configRouteBackendActionEdit + '/:idTbContent?', [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
...
router.put('/' + gSystemConfig.configRouteBackend + '/' + gSystemConfig.configRouteBackendContent + '/' + gSystemConfig.configRouteBackendActionEdit, [SyncSystemNS.FunctionsAuthentication.authenticationVerification_middleware('user_backend')], (req, res, next) => {
  // working, with the async block
  // Variables
  // ----------------------
  let tblContentID = '';
