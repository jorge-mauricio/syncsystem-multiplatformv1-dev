...
router.post('/' + gSystemConfig.configRouteAPI + '/' + gSystemConfig.configRouteAPILogin + '/', (req, res, next) => {
  ...
  let email = '';
  let password = '';
  let loginVerification = false; // eslint-disable-line
  let registerVerification = false; // eslint-disable-line
    ...
    try {
      ...
      if (email && password) {
        ...
        if (configAPIKey == SyncSystemNS.FunctionsCrypto.decryptValue(SyncSystemNS.FunctionsGeneric.contentMaskRead(apiKey, 'env'), 2)) {
          // Search for e-mail.
          // eslint-disable-next-line
          let resultRegisters = await SyncSystemNS.FunctionsDB.genericTableGet02(
          ...
          if (resultRegisters.length !== 0) {
            ...
            let tblRegistersUsername = ''; // eslint-disable-line
            let tblRegistersEmail = ''; // eslint-disable-line
            let tblRegistersPassword = '';
            let tblRegistersPasswordDecrypt = '';
            let tblRegistersPasswordHint = ''; // eslint-disable-line
            let tblRegistersPasswordLength = ''; // eslint-disable-line
...
router.post('/' + gSystemConfig.configRouteAPI + '/' + gSystemConfig.configRouteAPILogin + '/' + gSystemConfig.configRouteAPIDetails + '/:idTbRegisterCrypt', (req, res, next) => {
  // working, with the async block
  // Variables.
  // ----------------------
  ...
  let formfileFieldsReference = {}; // eslint-disable-line
  let resultsFunctionsFiles;
  let resultsFunctionsImageResize01; // eslint-disable-line
